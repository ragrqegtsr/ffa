...
wss.on('connection', (ws)=>{
  ws.on('message', (buf)=>{
    ...
    if(t==='host:create'){
      let code = '';
      let attempts = 0;
      // Boucle de sécurité pour garantir un code valide et unique
      while (!code || SESSIONS.has(code)) {
        code = codeId();
        attempts++;
        // Si la génération échoue 10x, on abandonne pour éviter une boucle infinie.
        if (attempts > 10) {
          console.error("Échec critique : Impossible de générer un code de session unique.");
          ws.send(JSON.stringify({ type: 'error', message: 'Erreur serveur lors de la création de la session.' }));
          return;
        }
      }
